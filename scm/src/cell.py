...
class cell():
    internal_code=0        #Code number for cell
    tic=0                  #Total tic or age
    status=1	           #1- alive  0-dead
    
    pdead=0                #deap provability
    ...
    minimal_running=0      #Min numer of cell alive
    ...
    def __init__(self, code, min_running):
       ...
       # self.max_running=int (config.get('Cell', 'max_number_running'))
        ...
        self.cell_processing_capacity=int (config.get('Cell', 'init_process_capacity'))
    ...
    def set_system_status(self,cpu2,mem2,io2):
        ...
        if (self.status==1):
            ...
            if (randint(0,99) < self.pdead) and (float(self.cpu_use) < float(self.dead_cpu_use)) :
                ...
                if (self.previous_cpu_use < self.cpu_use):
                    ...
                    self.action='s'
                    self.cell_processing_capacity-=self.cell_processing_capacity*self.var_processing_capacity/100
                    if (self.cell_processing_capacity<=0):
