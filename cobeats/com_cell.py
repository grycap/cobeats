...
class com_cell:
    ...
    def __init__(self, code, min_running,configfile):
        ...
        #self.pdupl_factor = int (config.get('Cell', 'duplprobability_factor'))
    ...
    #def actuate(self):
        ...
        if (self.status==1):
            '''#if cell is alive'''
            if (False and randint(0,99) < self.pdead) and (float(self.cpu_use) <= float(self.dead_cpu_use)) :
            ...
            elif (False and randint(0,99) < self.pdupl) and (self.cpu_use >= self.dupl_cpu_use):
                 '''#ACTION DUPLICATE - HORIZONTAL SCALING'''        
                 self.action='X'
                
            elif (False and randint(0,99) < self.pscav and self.cpu_use < self.dupl_cpu_use and self.cpu_use > self.dead_cpu_use):
